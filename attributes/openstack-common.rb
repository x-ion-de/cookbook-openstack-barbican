...
# key-manager to that cookbook's services list)
###################################################################
...
default['openstack']['mq']['key-manager']['service_type'] = node['openstack']['mq']['service_type']

default['openstack']['mq']['key-manager']['durable_queues'] =
  node['openstack']['mq']['durable_queues']
default['openstack']['mq']['key-manager']['auto_delete'] =
  node['openstack']['mq']['auto_delete']

rabbit_defaults.each do |key, val|
  default['openstack']['mq']['key-manager']['rabbit'][key.to_s] = val
end

###################################################################
# Database used by the OpenStack service
# (copied from openstack-common attributes/database.rb to avoid having to add
# key-manager to that cookbook (node['openstack']['common']['services'] in
